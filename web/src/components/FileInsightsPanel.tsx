...
import { cancelLifecycleManualReview, downgradeLifecycleRecord, fetchEvidence, fetchMemories, fetchPatterns } from '../api/client';
...
export function FileInsightsPanel({ projectId }: { projectId: string | null }) {
  ...
  const cancelManualReview = async (record: LifecycleRecord) => { if (!projectId || record.layer === 'pattern' || !confirm(`取消“${record.title || record.claim}”的自动晋级锁定？取消后不会立即升级，但后续满足条件时可以自动晋级。`)) return; await cancelLifecycleManualReview(projectId, record.layer, record.id); await load(); };
  ...
  return <>
    ...
    {open && createPortal(<div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" role="dialog" aria-modal="true"><div style={{ transform: `translate(${panelOffset.x}px, ${panelOffset.y}px)` }} className="flex max-h-[80vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl"><header onPointerDown={startDrag} onPointerMove={dragPanel} onPointerUp={stopDrag} onPointerCancel={stopDrag} className="flex cursor-grab touch-none items-start justify-between border-b px-6 py-5 active:cursor-grabbing"><div><h2 className="font-semibold text-gray-800">项目洞察</h2><p className="mt-1 text-xs text-gray-400">Evidence → Memory → Pattern；列表只显示简要标题，Trace 保留在后台可回溯。</p></div><button onClick={() => setOpen(false)} className="text-gray-400 hover:text-gray-700">✕</button></header><div className="flex items-center gap-1 bg-gray-50 px-6 py-3">{(Object.keys(label) as Layer[]).map(layer => <button key={layer} onClick={() => setTab(layer)} className={`rounded-lg px-3 py-1.5 text-sm ${tab === layer ? 'bg-white text-purple-700 shadow-sm' : 'text-gray-500'}`}>{label[layer]} ({data[layer].length})</button>)}<button onClick={() => void load()} className="ml-auto text-xs text-purple-600">{busy ? '刷新中…' : '刷新'}</button></div><main className="min-h-0 flex-1 overflow-y-auto p-5">{data[tab].map(record => <article key={record.id} className={`mb-3 rounded-xl border bg-white p-4 shadow-sm ${record.promotion_status === 'manual_review' ? 'border-amber-200' : 'border-gray-100'}`}><div className="mb-2 flex flex-wrap items-center gap-2 text-[11px] text-gray-400"><span>{record.category}</span><span>{record.domain}</span><span>权重 {Math.round(record.weight)}</span>{record.promotion_status === 'manual_review' && <span title={record.downgrade_reason} className="rounded-full bg-amber-50 px-2 py-0.5 font-medium text-amber-700">自动晋级已锁定</span>}<span className="ml-auto">{record.updated.slice(0, 16).replace('T', ' ')}</span></div><p className="text-sm leading-6 text-gray-700">{record.title || record.claim}</p>{record.promotion_status === 'manual_review' && <p className="mt-2 text-xs leading-5 text-amber-700">{record.downgrade_reason || '用户曾手动降低该记录层级，后续不会自动恢复。'}</p>}<div className="mt-3 flex gap-3 text-[11px] text-gray-400"><span>Trace {record.trace_id?.slice(0, 14) || '-'}</span><span>事件 {record.source_event_ids?.length || 0}</span>{record.support_count && <span>支持 {record.support_count}</span>}<span className="ml-auto flex gap-3">{record.promotion_status === 'manual_review' && <button onClick={() => void cancelManualReview(record)} className="text-purple-600 hover:text-purple-800">取消锁定</button>}{tab !== 'evidence' && <button onClick={() => void downgrade(record)} className="text-amber-700 hover:text-amber-900">降低层级</button>}</span></div></article>)}{!busy && data[tab].length === 0 && <p className="py-16 text-center text-sm text-gray-400">暂无 {label[tab]}。</p>}</main></div></div>, document.body)}
